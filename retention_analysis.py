...
# -*- coding: utf-8 -*-
"""
Анализ ретеншна пользователей на основе данных о платежах
"""
...
class RetentionAnalyzer:
    def __init__(self, data_file):
        """Инициализация анализатора ретеншна"""
    ...
    def load_data(self):
        ...
        # Преобразование datetime с обработкой различных форматов
    ...
    def calculate_cohort_retention(self):
        ...
        # Создаем pivot table для визуализации
        cohort_pivot = cohort_data.pivot(index='cohort_month', columns='period', values='users')
        
        # Заполняем пропуски нулями
        cohort_pivot = cohort_pivot.fillna(0)
        
        # Вычисляем ретеншн как процент от первоначального размера когорты
        cohort_sizes = cohort_pivot.iloc[:, 0]  # Размер когорты в день 0
    ...
    def calculate_rolling_retention(self, days=[1, 7, 30]):
        """Расчет rolling retention для разных периодов"""
        ...
        for day in days:
            ...
            # Для каждого дня вычисляем retention
            daily_retention = []
            dates = []
    ...
    def calculate_repeat_purchase_rate(self):
        """Расчет доли пользователей, совершивших повторные покупки"""
        print("\n=== АНАЛИЗ ПОВТОРНЫХ ПОКУПОК ===")
        
        # Считаем количество покупок для каждого пользователя
    ...
    def create_visualizations(self, retention_data, cohort_matrix):
        ...
        # 2. Heatmap когортного retention
        ...
        # 3. Распределение покупок по дням недели
        ...
        # 4. Распределение покупок по часам
        ...
        # 5. Тепловая карта активности по дням и часам
        ...
        # 6. Тренд общего количества покупок
    ...
    def generate_recommendations(self, retention_data, user_purchases):
        """Генерация рекомендаций по улучшению ретеншна"""
        print("\n" + "="*60)
    ...
    def run_full_analysis(self):
        """Запуск полного анализа ретеншна"""
...
def main():
    ...
    analyzer = RetentionAnalyzer('test-tasks/user_logs_paid_241024_250909.json')
